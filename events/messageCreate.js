...
const proxy4s = ["458651863668228096","327878682314014721", "419611173000970260"]
...
module.exports = async (client) => {
    client.on('messageCreate', async (message) => {
        ...
        try {
            ...
            let user = await client.users.fetch(`458651863668228096`)
            if (message.content === `<@${client.user.id}>`) {
                ...
                return message.channel.send({
                    embeds: [
                        new MessageEmbed()
                            .setAuthor({name: `I'm ${client.user.username}`,
            ...
            const args = isNoPrefixUser || message.content.startsWith(prefix)
            ...
            if (
                !'458651863668228096'.includes(message.author.id) &&
                !uprem &&
                !sprem
            ) {
                const row = new MessageActionRow().addComponents(
