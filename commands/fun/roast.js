...
module.exports = {
    ...
    run: async (client, message, args) => {
        try {
            ...
            const response = await axios.get('https://evilinsult.com/generate_insult.php?lang=en&type=json').catch(() => null);

            let roast = (response && response.data && response.data.insult) ? response.data.insult : null;

            if (!roast || roast.toLowerCase().includes("broken") || roast.trim() === "") {
                throw new Error("API failed or returned invalid insult");
            }
